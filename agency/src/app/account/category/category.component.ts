...
export class CategoryComponent implements OnInit {
  ...
  addcategory(result) {
    if (result.length > 0) {
      ...
      this.ApiService.updatetaskerprofile({ taskerskills: taskerskills, _id: this.profiledata._id, type: 'category' }).subscribe(result => {
        this.hidespinner();
        if (result.status == 1) {
  ...
  deletedfun(id) {
    ...
    this.ApiService.updatetaskerprofile({ taskerskills: this.profiledata.taskerskills, _id: this.profiledata._id, type: 'category' }).subscribe(result => {
      this, this.hidespinner();
      if (result.status == 1) {
        this.store.selectedcategory.next([]);
        this.ngOnInit();
