...
@Component({
  selector: 'app-subcategorieslist',
  templateUrl: './subcategorieslist.component.html',
  styleUrls: ['./subcategorieslist.component.scss']
})
export class SubcategorieslistComponent implements OnInit {
  ...
  editurl: string = '/app/categories/subedit';
  viewurl: string = '/app/categories/subedit';
  addurl: string = '/app/categories/subcat-add';
  ...
  constructor(
    ...
    if (this.curentUser && this.curentUser.user_details && this.curentUser.user_details.role == "subadmin") {
      if (this.router.url == '/app/categories/sublist') {
  ...
  ngOnInit(): void {
    ...
    this.Apiservice.CommonApi('post', Apiconfig.subcategorylist, data).subscribe((results: any) => {
    	if (results.status === 1) {
        this.hidespinner();
  ...
  loadsettings(){
    this.settings = {
      ...
      columns: {
        ...
        //   title: 'Parent',
        //   filter: true,
        //   valuePrepareFunction: (categoryName) => {
