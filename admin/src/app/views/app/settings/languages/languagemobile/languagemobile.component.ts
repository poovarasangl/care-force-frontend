...
@Component({
  selector: 'app-languagemobile',
  templateUrl: './languagemobile.component.html',
  styleUrls: ['./languagemobile.component.scss']
})
export class LanguagemobileComponent implements OnInit {
  editid: any;
  ...
  addurl: string = '/app/settings/languages/mobile-add';
  ...
  ngOnInit(): void {
    ...
    if (this.editid) {
      ...
      this.Apiservice.CommonApi('post', Apiconfig.languagemobilelist, data).subscribe(
        (results) => {
          if (results.status == 1) {
            this.hidespinner();
  ...
  onPageChange(event) {
    ...
    this.Apiservice.CommonApi('post', Apiconfig.languagemobilelist, data).subscribe((results: any) => {
      if (results.status == 1) {
        this.hidespinner();
        const objectArray = Object.entries(results.response.data);
        var keys = [];
  ...
  submiteBtn() {
    ...
    if (this.changedValue) {
      this.Apiservice.CommonApi('post', Apiconfig.languagemanagesave, { data: this.changedValue, id: this.editid,type: "mobile" }).subscribe(
        (result) => {
          if (result.status == 1) {
            this.hidespinner();
