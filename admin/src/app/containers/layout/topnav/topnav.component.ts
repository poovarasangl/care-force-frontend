...
export class TopnavComponent implements OnInit, OnDestroy {
	...
	constructor(private sidebarService: SidebarService, private router: Router, private langService: LangService, private AuthenticationService: AuthenticationService,
	...
	ngOnInit() {
		this.AdminService.CommonApi('get', Apiconfig.getSettingsData + '?alias=general', {}).subscribe(
			(results) => {
				this.settingData = results.response.settings;
				this.sitelogo = this.settingData ? this.imageUrl + this.settingData.logo : '';
				this.baseurl = this.settingData.site_url;
			});
		this.AdminService.CommonApi('get', Apiconfig.getpendingtaskerlist, {}).subscribe(
			(results) => {
				if (results.status == 1) {
					this.pendingTasker = results.response;
				}
			},
			(error) => {
				console.log(error);
			})
		if (this.AuthenticationService.currentUserValue.user_details) {
			this.displayName = this.AuthenticationService.currentUserValue.user_details.username;
		}
	...
	dateDifference(value) {
		console.log(moment(new Date(value)).diff(moment(new Date()), 'months', true));
	}
}
