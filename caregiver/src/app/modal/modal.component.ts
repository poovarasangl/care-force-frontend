...
export class ModalComponent implements OnInit {
  ...
  Categorychange(event) {
    this.categoryedittaskerForm.controls['rate'].setValue(100);
  ...
  categoryeditsubmit() {
    console.log("this.categoryedittaskerForm", this.categoryedittaskerForm)
    if (this.categoryedittaskerForm.invalid || this.commision != '') {
      // console.log("00000000000000", this.commision)
      return;
    }
    let data = [] as any;
    ...
    // this.store.selectedcategorfvy.next(data);
    this.modalcontend.siFn(data);
  }
