...
export class DatePickerDirectiveComponent implements OnInit {
  ...
  clickdate(day, side) {
    ...
    if (this.singleDatePicker) {     
      this.setEndDate(this.startDate);
      this.focusedInput = "start";
      this.dateSelected.emit(this.startDate.toLocaleDateString('en-GB'));      
    }
    this.updateView();
  }
  rangeClick(range) {
  ...
  setEndDate(endDate) {    
    ...
    this.displayEndDate = this.endDate.toLocaleDateString('en-GB');
    this.tmpEndDate = null;    
  }
  applyClick() {
    this.dateSelected.emit(this.startDate.toLocaleDateString('en-GB') + " - " + this.endDate.toLocaleDateString('en-GB'));
  }
  cancelClick() {
